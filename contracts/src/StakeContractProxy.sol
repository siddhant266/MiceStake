...
contract StakeContractProxy is Proxy, Ownable {
    ...
    // This function demonstrates how proxy manually forwards calls
    // In real usage, fallback() automatically handles this
    function getRewards() public returns (uint256 reward) {
        bytes memory data = abi.encodeWithSignature("getRewards()");
        (bool success, bytes memory returnData) = _getImplementation()
            .delegatecall(data);
